#!/usr/bin/env python3
"""
DenseNet-121 trainer for our fossil project (simple & readable).

- Assumes images are in either:
    A) data/images/<ClassName>/*.jpg  (script will auto‑split train/val/test), or
    B) data/images/{train,val,test}/<ClassName>/*.jpg

- Minimal logging (prints + tiny CSV), saves best checkpoint.
- Written by MEL (Week 8) so it looks like a normal class project script.
"""
...
def has_split_layout(root: str) -> bool:
...
def make_loaders(data_dir: str, img_size: int, batch: int, workers: int,
    ...
    if has_split_layout(data_dir):
        ...
        # One big folder: split once, reuse
    ...
    return train_ld, val_ld, test_ld, class_to_idx, num_classes
...
# ---- main -------------------------------------------------------------------
def main():
    p = argparse.ArgumentParser("DenseNet-121 trainer")
    p.add_argument("--data_dir", required=True)
    p.add_argument("--output_dir", default="runs/densenet121")
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f); writer.writerow(["epoch", "train_loss", "train_acc", "val_loss", "val_acc"])  # header
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if va_acc >= best_val:
            ...
            torch.save({"state_dict": model.state_dict(),
                        ...
                        "image_size": args.image_size}, best_ckpt)
